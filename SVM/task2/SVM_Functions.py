    ...
    if model['kernelFunction']=='linear':
        ...
        for i in range(m):
            ...
            for j in range(model['X'].shape[0]):
                ...
                gaussianKernelSub(X[i,:].T, model['X'][j,:].T, sigma=sigma)
...
def visualizeBoundaryGaussian(X, y, model, sigma, c):
    """
    分类边界可视化
    ...
    plt.savefig('./img/c='+str(c)+';sigma='+str(sigma)+'.png')
